...
- (TextFieldInputView *)inputTextView{
    ...
    if (!_inputTextView) {
        _inputTextView = [[TextFieldInputView alloc]initWithFrame:CGRectMake(0, SCREEN_WIDTH, SCREEN_WIDTH, 67)];
        
        LRWeakSelf(self);
        _inputTextView.endBlock = ^(NSString * _Nonnull text) {
            LRStrongSelf(self);
            if (self.writeBlock) {
                self.writeBlock(text);
            }
