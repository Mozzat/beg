...
- (void)initSubviews{
    ...
    [self.middleView addSubview:self.replyComment];
    [self.replyComment mas_makeConstraints:^(MASConstraintMaker *make) {
        
        make.left.top.equalTo(self.middleView).offset(10);
        make.right.equalTo(self.contentView).offset(-20);
        
    }];
    
    [self.middleView addSubview:self.openBtn];
    [self.openBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        
        make.top.equalTo(self.replyComment.mas_bottom).offset(10);
        make.right.equalTo(self.middleView).offset(-10);
        make.bottom.equalTo(self.middleView).offset(-10);
...
- (UIView *)middleView{
    ...
    if (!_middleView) {
        ...
        _middleView.backgroundColor = HexColor(@"f5f5f5");
        [_middleView rounded:4];
...
- (UIButton *)openBtn{
    ...
    if (!_openBtn) {
        ...
        [_openBtn setTitle:@"查看x条回复" forState:UIControlStateNormal];
        [_openBtn setTitleColor:redColor() forState:UIControlStateNormal];
        _openBtn.titleLabel.font = Font12();
